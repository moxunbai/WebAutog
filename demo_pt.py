...
class RayTracingRender():
    ...
    def field_declare(self):
        self.color_buffer = ti.Vector.field(3, dtype=ti.f32)
        self.count_var = ti.field(ti.i32)
        self.tonemapped_buffer = ti.Vector.field(3, dtype=ti.f32)

    def setup_data_cpu(self):
        ti.root.dense(ti.ij,  self.res ).place(self.color_buffer,self.tonemapped_buffer)
        ti.root.dense(ti.i,1).place(self.count_var)
    ...
    def render(self):
        for u, v in self.color_buffer:
            ...
            pos = camera_pos
            cur_iter = self.count_var[0]
            str_x, str_y = (cur_iter / stratify_res), (cur_iter % stratify_res)
            ray_dir = ti.Vector([
                (2 * fov * (u + (str_x + ti.random()) * inv_stratify) / self.res[1] -
                fov * aspect_ratio - 1e-5),
                (2 * fov * (v + (str_y + ti.random()) * inv_stratify) / self.res[1] -
                fov - 1e-5),
                -1.0,
            ])
            ...
            self.color_buffer[u, v] += acc_color
    ...
    def tonemap(self,accumulated: ti.f32):
        for i, j in self.tonemapped_buffer:
            self.tonemapped_buffer[i, j] = ti.sqrt(self.color_buffer[i, j] / accumulated *
                                            100.0)
...
# renderer = RayTracingRender()

# def main():
#     gui = ti.GUI('Cornell Box', renderer.res, fast_gui=True)
#     gui.fps_limit = 300
#     last_t = time.time()
#     i = 0
#     while gui.running:
...
#             )
#             last_t = time.time()
#             gui.set_image(renderer.tonemapped_buffer)
#             gui.show()
#         i += 1
